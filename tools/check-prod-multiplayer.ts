...
async function runCoop2(browser: Browser): Promise<BrowserCaseResult> {
  ...
  try {
    await Promise.all(clients.map((client, slot) => client.page.goto(coopUrl(session, slot), { waitUntil: 'domcontentloaded' })));
...
async function runDeathmatchPrejoined2(browser: Browser): Promise<BrowserCaseResult> {
  ...
  try {
    await joiner.page.goto(deathmatchPeerUrl(session, 1, 2, { aiFill: true }), { waitUntil: 'domcontentloaded' });
    await wait(1_800);
    await host.page.goto(deathmatchPeerUrl(session, 0, 2, { aiFill: true, humanSlots: '0,1' }), { waitUntil: 'domcontentloaded' });
    await Promise.all(clients.map((client) => waitPlaying(client, 2, 0)));
...
async function runDeathmatchLate2(browser: Browser): Promise<BrowserCaseResult> {
  ...
  const clients = [host, joiner];
  try {
    await host.page.goto(deathmatchPeerUrl(session, 0, 2, { aiFill: true, humanSlots: '0' }), { waitUntil: 'domcontentloaded' });
...
async function runDeathmatchAllHuman4(browser: Browser): Promise<BrowserCaseResult> {
  ...
  try {
    await Promise.all(clients.map((client, slot) => client.page.goto(deathmatchPeerUrl(session, slot, 4), { waitUntil: 'domcontentloaded' })));
    await Promise.all(clients.map((client) => waitPlaying(client, 4, 0)));
    await watcher.page.goto(spectateUrl(session, 4), { waitUntil: 'domcontentloaded' });
...
async function runDeathmatchAiFill(browser: Browser, players: number): Promise<BrowserCaseResult> {
  ...
  try {
    await clients[0].page.goto(deathmatchPeerUrl(session, 0, players, { aiFill: true, humanSlots: '0,1' }), { waitUntil: 'domcontentloaded' });
    await clients[1].page.goto(deathmatchPeerUrl(session, 1, players, { aiFill: true }), { waitUntil: 'domcontentloaded' });
