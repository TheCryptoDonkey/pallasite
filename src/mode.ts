/**
 * Run mode: campaign, drift, arena, deathmatch, sanctum, duel, plus a bossrush stub.
 *
...
export type RunMode = 'campaign' | 'drift' | 'bossrush' | 'arena' | 'deathmatch' | 'sanctum' | 'defender' | 'duel';
...
export function getStoredMode(): RunMode {
  try {
    ...
    if (v === 'campaign' || v === 'drift' || v === 'bossrush' || v === 'arena' || v === 'deathmatch' || v === 'sanctum' || v === 'duel') return v;
    if (v === 'defender') {
...
export function isModeReady(m: RunMode): boolean {
  return m === 'campaign' || m === 'drift' || m === 'arena' || m === 'deathmatch' || m === 'sanctum' || m === 'defender';
}
...
export const MODE_LIST: readonly ModeInfo[] = [
  ...
  { id: 'deathmatch', label: 'DEATHMATCH', hint: 'Large space. Radar, cover, AI rivals.',                 ready: true },
