...
/**
 * Build a zap request and fetch a BOLT11 invoice for the dev's lightning address.
 *
 * If `session` is null or the endpoint doesn't support nostr, falls back to a
 * plain LNURL pay invoice (still gets the sats, just no zap receipt).
 */
export async function requestZapInvoice(
  ...
  const { url, meta } = await resolveLightningAddress(DEV.lightningAddress);
  ...
  if (canZap && session) {
    ...
    const zapRequest = await buildZapRequest(session, DEV.pubkey, amountMsats, comment, lnurlEncoded, relays);
