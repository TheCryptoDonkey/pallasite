...
import { render, preloadBackground, setRenderMode, getRenderMode, type RenderModeInfo } from './render.js';
...
function renderLiveTheatre(input: LiveTheatreInput): void {
  ...
  // Internal resolution locked to the world size (×dpr): the shared
  // render() draws in world coords under a retro-mode transform, so the
  // canvas must be a whole-world surface. CSS still shows it at
  // cssWidth × cssHeight.
  ...
  const cleanup = (): void => {
    ...
    window.removeEventListener('keydown', onKey);
    // Restore the render mode the game loop expects — the theatre
    // overwrote render.ts's module-level mode on every frame.
    setRenderMode(savedRenderMode);
    setMeshTierClamp(null);
  };
  const onKey = (e: KeyboardEvent): void => {
  ...
  // Theatre canvas runs in the game's retro render mode: world coords
  // map to device pixels through a persistent dpr transform, so the
  // shared render() from render.ts fills the canvas exactly.
  c2d.setTransform(dpr, 0, 0, dpr, 0, 0);
  const theatreRenderMode: RenderModeInfo = {
    kind: 'retro', vw: cssWidth, vh: cssHeight, dpr,
    scale: 1, tx: 0, ty: 0, insets: { top: 0, right: 0, bottom: 0, left: 0 },
  };
  // render.ts holds the render mode in a module-level slot the game loop
  // also reads — snapshot it so closing the theatre restores it.
  const savedRenderMode = getRenderMode();
  // One persistent synthetic GameState, rewritten each frame from the
  // interpolated wire frames and handed to render().
  const theatreState = createTheatreState();
  // The theatre renders through the shared render() but cannot drive the
  // WebGL mesh overlay (its canvas is bound to the game). Clamp mesh down
  // to vector — pure line-art, no texture or overlay dependency — so a
  // mesh-tier watcher always sees the world drawn.
  setMeshTierClamp('vector');
  ...
  const tick = (): void => {
    ...
    // Background, starfield and every entity are drawn by the shared
    // render() below — see the populate + render call.
    ...
    // Decay the viewer-side cosmetic pools (spawned by SFX events).
    // render() draws them; their motion + fade is ticked here so they
    // stay independent of the wire cadence.
    const dt = 1 / 60;
    ...
    // Synthesise a GameState from the interpolated frame pair and draw
    // it through the shared game renderer — the same render() the game
    // uses, so the theatre inherits vector / shaded fidelity.
    ...
    // The warp banner + status overlays draw in screen pixels, so step
    // out of the world transform render() leaves on the context.
    c2d.save();
    ...
    // Restore the world transform for the next frame's render().
    c2d.restore();
  };
  rafId = requestAnimationFrame(tick);
}
